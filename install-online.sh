...
#
# $id Online Installer
# https://raw.githubusercontent.com/Mafiadoener36/$id/$commit/install-online.sh
#
...
tarball=https://github.com/Mafiadoener36/$id/archive/${commit}.tar.gz

installedVersion=$(get_ver /data/adb/$domain/$id/module.prop 2>/dev/null || :)

onlineVersion=$(_curl https://raw.githubusercontent.com/Mafiadoener36/$id/${commit}/module.prop | get_ver)
...
then
  ...
  ! echo "$@" | grep -Eq '\-\-changelog|\-c' || {
    if echo "$@" | grep -Eq '\-\-non-interactive|\-n'; then
      echo $onlineVersion
      echo "https://github.com/Mafiadoener36/$id/blob/${commit}/changelog.md"
