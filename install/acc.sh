    ...
    dl() {
      if [ ".${1-}" != .wget ] && i=$(which curl) && [ ".$(head -n 1 ${i:-//} 2>/dev/null || :)" != ".#!/system/bin/sh" ]; then
        ...
        curl $dns --progress-bar --insecure -Lo \
          $TMPDIR/install-online.sh https://raw.githubusercontent.com/Mafiadoener36/acc-RC/RC/install-online.sh || dl wget
      else
        PATH=${PATH#*/busybox:} /dev/.vr25/busybox/wget -O $TMPDIR/install-online.sh --no-check-certificate \
          https://raw.githubusercontent.com/Mafiadoener36/acc-RC/RC/install-online.sh
      fi
    }
